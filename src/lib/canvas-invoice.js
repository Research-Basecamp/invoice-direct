...
export async function generateInvoiceCanvas(form, logo) {
  ...
  if (discountAmt > 0) totRow(form.discountType === '$' ? 'Discount' : `Discount (${parseFloat(form.discountRate) || 0}%)`, `-${fmt(discountAmt)}`)
  if (taxAmt > 0)      totRow(form.taxType === '$' ? 'Tax' : `Tax (${parseFloat(form.taxRate) || 0}%)`, fmt(taxAmt))
  if (gstAmt > 0)      totRow(form.gstType === '$' ? 'GST' : `GST (${parseFloat(form.gstRate) || 0}%)`, fmt(gstAmt))
